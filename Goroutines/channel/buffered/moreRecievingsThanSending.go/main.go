...
func send(ch chan string) {
	// Print the initial length of the channel (should be 0 initially).
	fmt.Printf("Channel length before sending anything: %v\n", len(ch))

	// Loop to send 2 messages into the channel (instead of 3).
	for i := 1; i < 3; i++ { // The loop runs twice (i = 1, 2)
...
func receive(ch chan string) {
	// Loop to receive 5 messages, but only 2 messages exist in the channel.
	// This will lead to a deadlock because the channel does not contain enough messages.
	for i := 0; i < 5; i++ { // This loop runs 5 times but only 2 messages are available
...
// ✅ Always ensure that the receiver doesn’t read more messages than the sender sends.
// ✅ If you don’t know how many messages will be sent, close the channel and use for range.
// ✅ Buffered channels help prevent blocking, but they still require proper synchronization.
